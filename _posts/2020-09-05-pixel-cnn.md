...
<p class="textblock">
The softmax loss is as follows:

  $-\frac{1}{N \cdot 3 \cdot H \cdot W}\log_e\prod_{i=0}^{N-1}\prod_{x=0}^{H-1}\prod_{y=0}^{W-1}[insert prob here]$

This is the normalized NLL in nats averaged across the images. Here the log has base of $e$ whereas to get it in bits per dimension we need to use a base of 2. Since log_2(x) = log_e(x) / log_e(2), simply dividing the softmax loss by $\log2$ gives you the average NLL in bits normalized by the dimensionality of the images.
</p>
</div>
